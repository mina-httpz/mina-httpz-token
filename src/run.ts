...
const decryptPrivateKey = (
  ...
  const decipher = crypto.createDecipheriv(
    ...
    // Extract IV from the encrypted data (first 24 characters in base64 = 16 bytes)
    Buffer.from(encryptedPrivateKey.slice(0, 24), "base64")
  );
...
if (func == "deployToken") {
  ...
  try {
    ...
    let tx = await Mina.transaction(
      ...
      async () => {
        AccountUpdate.fundNewAccount(feepayerAddress, 1);
        let userAU = await vaultContract.claim(
          PublicKey.fromBase58(
            "B62qkdmPUHBi4HBiGFZdmL5TyCF8xAiCbeU8xCyD4c2L1q4zrsDH1Pb"
          )
        );
